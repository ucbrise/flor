...
with flor.Experiment("twitter_demo") as ex:
    ...
    tweets = ex.artifact('tweets.csv', 'tweets')
    ...
    training_tweets = ex.artifact('training_tweets.pkl', 'training_data', do_split)
    testing_tweets = ex.artifact('testing_tweets.pkl', 'test_data', do_split)

    alpha = ex.literalForEach(np.linspace(0.0, 1.0, 8).tolist(), 'alpha')
    # alpha.forEach()

    from train_model import train
    do_train = ex.action(train, [training_tweets, alpha])
    model = ex.artifact('model.pkl', 'model', do_train)
