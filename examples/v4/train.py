...
flor.set_ckpt_interval(flor.arg("ckpt_interval_s", 60.0))
...
# skipped/narrowed by flor during replay (`--replay_flor step=...`); the outer
# "epoch" gives one ctx-tagged time::iter per epoch. Plain `for` still works
# (see the inner loop in v3) -- you just lose the replay-narrowing hint for
# that scope. No `with flor.checkpointing(...):` block: per-epoch torch.save
# below is piggy-backed into .flor/obj_store/ automatically.
