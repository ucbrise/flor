...
class Transformer(ast.NodeTransformer):
    ...
    def _proc_imports(self, node, header_obj):
        """
        ...
        if contains_imports:
            ...
            heads = header_obj.get_heads()

            heads.extend(postfix)
            prefix.extend(heads)
            node.body = prefix
            node.body.extend(header_obj.get_foot())
        else:
            heads = header_obj.get_heads()
            heads.extend(node.body)
            node.body = heads
        ...
        for field, old_value in ast.iter_fields(node):
            if isinstance(old_value, list):
                ...
                if self.header_license:
                    header_obj = self._Header(ClientRoot(self.filepath, self.relative_counter))
                ...
                if header_obj is not None:
                    ...
                    self._proc_imports(node, header_obj)
